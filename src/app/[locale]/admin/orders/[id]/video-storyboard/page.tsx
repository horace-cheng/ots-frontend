...
export default function VideoStoryboardPage() {
  ...
  const [voiceSettings, setVoiceSettings] = useState<{ voiceId: string; speakingRate: number; shortPause: number; longPause: number }>({
    voiceId: TRACK_DEFAULT_VOICES['tai-lo'], speakingRate: 1.0, shortPause: 150, longPause: 450,
  })
      ...
      if (r.materials) {
        ...
        if (s) {
          setVoiceSettings({
            ...
            speakingRate: (s as any).speaking_rate ?? 1.0,
            shortPause: (s as any).short_pause_duration ?? 150,
            longPause: (s as any).long_pause_duration ?? 450,
          })
        }
      } else {
  ...
  const handleSceneTts = useCallback(async (chIdx: number, sIdx: number, track: Track) => {
    ...
    const text = scene.tracks?.[track]?.narration_text || ''
    if (!text) { setMessage('旁白文字為空'); return }

    setSceneAssets(prev => ({ ...prev, [key]: { ...prev[key], audioState: 'loading', audioUrl: prev[key]?.audioUrl || '' } }))
    setMessage('')
    try {
      const r = await adminSceneTts(id, chIdx, sIdx, text, voiceSettings.voiceId, voiceSettings.speakingRate, track, voiceSettings.shortPause, voiceSettings.longPause)
  ...
  const handleSceneReferenceImage = useCallback(async (chIdx: number, sIdx: number) => {
    if (!materials) return
    const k = assetKey(chIdx, sIdx, 'tai-lo')
    setMessage('')
    try {
      setSceneAssets(prev => ({ ...prev, [k]: { ...prev[k], imageState: 'loading' as AssetState } }))
      const r = await adminSceneReferenceImage(id, chIdx, sIdx)
      setSceneAssets(prev => ({ ...prev, [k]: { ...prev[k], imageUrl: r.image_data_url, imageState: 'done' as AssetState } }))
      setMessage('參考圖片已產生')
    } catch (e: unknown) {
      setSceneAssets(prev => ({ ...prev, [k]: { ...prev[k], imageState: 'error' as AssetState } }))
    ...
    <div className="space-y-4 fade-up">
      ...
      {showSettings && (
        <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-4 items-end">
            <div>
              <label className="text-xs text-mist block mb-1">語者</label>
            ...
            <div>
              <label className="text-xs text-mist block mb-1">語速 — {voiceSettings.speakingRate.toFixed(1)}</label>
              <input type="range" min="0.25" max="4.0" step="0.25"
                value={voiceSettings.speakingRate}
                onChange={e => { const v = parseFloat(e.target.value); setVoiceSettings(prev => ({ ...prev, speakingRate: v })); setMaterials(prev => prev ? { ...prev, settings: { ...prev.settings, speaking_rate: v } } : prev) }}
                className="w-28 accent-gold" />
            </div>
            <div>
              <label className="text-xs text-mist block mb-1">短停頓 — {voiceSettings.shortPause}ms</label>
              <input type="range" min="0" max="500" step="10"
                value={voiceSettings.shortPause}
                onChange={e => { const v = parseInt(e.target.value); setVoiceSettings(prev => ({ ...prev, shortPause: v })); setMaterials(prev => prev ? { ...prev, settings: { ...prev.settings, short_pause_duration: v } } : prev) }}
                className="w-28 accent-gold" />
            </div>
            <div>
              <label className="text-xs text-mist block mb-1">長停頓 — {voiceSettings.longPause}ms</label>
              <input type="range" min="0" max="1000" step="10"
                value={voiceSettings.longPause}
                onChange={e => { const v = parseInt(e.target.value); setVoiceSettings(prev => ({ ...prev, longPause: v })); setMaterials(prev => prev ? { ...prev, settings: { ...prev.settings, long_pause_duration: v } } : prev) }}
                className="w-28 accent-gold" />
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-thin">
        {materials.chapters.map((ch: VideoChapter, ci: number) => {
          const ck = `${ch.chapter_index}_tai-lo`
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={batchLoading[`tts_${chapter.chapter_index}_tai-lo`]}
                ...
                {batchLoading[`tts_${chapter.chapter_index}_tai-lo`] ? '⏳' : '🔊 台語語音'}
              </button>
              {(() => {
                    ...
                    {hasVideo && (
                      <>
                        ...
                        {chapterSrtUrls[chKey] && (
                          <button onClick={() => openSrtEditor(chapter.chapter_index, 'tai-lo')}
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex items-center gap-1">
                          {(() => {
                            ...
                            return (
                              <>
                                <button
                                  onClick={() => handleSceneTts(chapter.chapter_index, sIdx, 'tai-lo')}
                        ...
                        <div className="flex items-center gap-1">
                          {(() => {
                            ...
                            const fileInputId = `video-upload-${chapter.chapter_index}-${sIdx}-tai-lo`
                            return (
                              <>
                                <button
                                  onClick={() => noRef ? undefined : handleSceneVideo(chapter.chapter_index, sIdx, 'tai-lo')}
                                ...
                                <input id={fileInputId} type="file" accept="video/mp4,video/webm,video/quicktime"
                                  ...
                                  onChange={e => {
                                    ...
                                    handleSceneVideoUpload(chapter.chapter_index, sIdx, 'tai-lo', mergeAudio, f)
                                    e.target.value = ''
                                  }} />
                              </>
                            )
